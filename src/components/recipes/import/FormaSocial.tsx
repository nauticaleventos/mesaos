import { useState } from 'react'
import { Share2 } from 'lucide-react'
import { importFromSocial, type RecipeImport } from '../../../lib/claudeImport'
import { ModalWrap } from './ImportModal'

interface Props { onExtracted: (r: RecipeImport) => void; onBack: () => void }

export default function FormaSocial({ onExtracted, onBack }: Props) {
  const [url, setUrl]         = useState('')
  const [loading, setLoading] = useState(false)
  const [error, setError]     = useState<string | null>(null)

  const procesar = async () => {
    if (!url.trim()) return
    setError(null); setLoading(true)
    try { onExtracted(await importFromSocial(url.trim())) }
    catch (e) { setError(e instanceof Error ? e.message : 'Error al analizar') }
    setLoading(false)
  }

  return (
    <ModalWrap titulo="Desde redes sociales">
      <div className="flex flex-col gap-5">
        <div className="flex flex-col items-center gap-3 pt-2 pb-1">
          <div className="w-16 h-16 rounded-2xl flex items-center justify-center" style={{ background: '#FFCDB2' }}>
            <Share2 size={32} color="#E76F51" />
          </div>
          <div className="text-center">
            <p className="font-semibold text-text">Link del post o video</p>
            <p className="text-sm text-muted mt-1">
              Pegá el link de Instagram, TikTok, YouTube o Facebook.
              También podés usar el botón "Compartir" de la red social y elegir mesa.os.
            </p>
          </div>
        </div>

        {/* Tips de plataformas */}
        <div className="grid grid-cols-2 gap-2">
          {[
            { nombre: 'Instagram', emoji: '📸', tip: 'Botón ··· → Compartir → Copiar link' },
            { nombre: 'TikTok',    emoji: '🎵', tip: 'Compartir → Copiar link' },
            { nombre: 'YouTube',   emoji: '▶️', tip: 'Compartir → Copiar link' },
            { nombre: 'Facebook',  emoji: '👥', tip: 'Compartir → Copiar link' },
          ].map(({ nombre, emoji, tip }) => (
            <div key={nombre} className="p-3 rounded-xl border border-border bg-white">
              <p className="text-sm font-medium text-text">{emoji} {nombre}</p>
              <p className="text-xs text-muted mt-0.5">{tip}</p>
            </div>
          ))}
        </div>

        <div>
          <label className="input-label">Link del post o video</label>
          <input type="url" value={url} onChange={e => setUrl(e.target.value)} autoFocus
            placeholder="https://www.instagram.com/reel/..." />
        </div>

        {error && <div className="p-3 rounded-xl bg-red-50 border border-red-200"><p className="text-sm text-error">{error}</p></div>}

        {loading ? (
          <div className="flex items-center gap-3 py-4 justify-center">
            <div className="flex gap-1">{[0,150,300].map(d => <span key={d} className="w-2 h-2 rounded-full bg-accent animate-bounce" style={{ animationDelay: `${d}ms` }} />)}</div>
            <p className="text-sm text-muted">Analizando receta… 🤖</p>
          </div>
        ) : (
          <button onClick={procesar} disabled={!url.trim()} className="btn-primary">Analizar receta</button>
        )}
        <button onClick={onBack} className="btn-ghost">← Volver</button>
      </div>
    </ModalWrap>
  )
}
